...
//		List<Phim> newP = list.stream().filter(item -> maKM.equals(item.getMaKM())).collect(Collectors.toList());
	...
	public Integer deleteNews(String maKM) {
		// kiểm tra trạng thái của giao dịch hiện tại trước khi bắt đầu một giao dịch
		// mới.
	...
	public String insert(ModelMap model, @ModelAttribute("promotion") KhuyenMai promotion, BindingResult result,
			BindingResult errors, @RequestParam("tenKM") String tenKM, @RequestParam("ngayBatDau") String ngayBatDau,
			@RequestParam("ngayKetThuc") String ngayKetThuc, @RequestParam("maTT") Integer maTT,
			@RequestParam("mucGiamGia") String mucGiamGia, @RequestParam("moTa") String moTa, BindingResult erors,
		...
		if (mucGiamGia.matches(".*[a-zA-Z].*")) {
			redirectAttributes.addFlashAttribute("message",
					new Message("error", "Mức giảm giá không được chứa ký tự!"));
			return "redirect:/admin/promotion.htm";
		}

		if (ngayBatDauDate.before(new Date())) {
			redirectAttributes.addFlashAttribute("message",
					new Message("error", "Ngày bắt đầu khuyến mãi không hợp lệ"));
			return "redirect:/admin/promotion.htm";
		}

		if (ngayKetThucDate.before(new Date())) {
			redirectAttributes.addFlashAttribute("message",
					new Message("error", "Ngày kết thúc khuyến mãi không hợp lệ"));
			return "redirect:/admin/promotion.htm";
		}

		if (ngayKetThucDate.before(ngayBatDauDate)) {
			redirectAttributes.addFlashAttribute("message",
					new Message("error", "Ngày bắt đầu phải trước ngày kết thúc, không hợp lệ"));
			return "redirect:/admin/promotion.htm";
		}
		if (promotion.getMucGiamGia() == null) {
			redirectAttributes.addFlashAttribute("message", new Message("error", "Mức giảm giá phải nhập số!"));
			return "redirect:/admin/promotion.htm";
		}
		try {
			Session session = factory.openSession();
	...
	public String deletePromotion(ModelMap model, @PathVariable("maKM") String maKM,
		...
		try {
			...
			if (temp == 1) {
				...
				redirectAttributes.addFlashAttribute("message",
						new Message("error", "Xóa thất bại, Mã khuyến mãi đang được áp dụng cho phim khác!"));
	...
	public String updatePromotion(ModelMap model, @ModelAttribute("promotion") KhuyenMai promotion,
			BindingResult result, BindingResult errors, @RequestParam("maKM") String maKM,
			@RequestParam("link") String link, @RequestParam("tenKM") String tenKM,
...
//			if (temp == 1) {
			...
			promotion.setMucGiamGia(Integer.parseInt(mucGiamGia));
